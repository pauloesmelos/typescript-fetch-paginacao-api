...
type TuplaDay = [string, number]; // tupla com maior dia e quantidade vendas ['Terça', 54]
const handleFetch = async () => {
   const data = await fetchAPI<TransactionAPI[]>('https://api.origamid.dev/json/transacoes.json');
...
function showTable(selector: string,data: Array<Transaction>) {
   const tbody = document.querySelector<HTMLElement>(selector);
   if(tbody && data) {
      data.map((element) => {
         tbody.innerHTML += `
            <tr class="tr">
...
function showStatistic(data: Array<Transaction>) { // total
...
function showOtherStatistics(data: Transaction[]) { // demais estatisticas
   let statistic = new Estatistica(data);
   ...
   Object.keys(days).forEach((e: string,i) => {
      if(container1) {
 ...
 */
function showType(divSelector: string, option: string, object: any, data: Transaction[]): void {
   // função extremamente dinâmica que pode ser usado de duas maneiras diferentes
   const divContainer = document.querySelector<HTMLElement>(divSelector);
   let statistic = new Estatistica(data);
...
*/
function showDay(day: string, object: Days) {
...
function showMoreSales(selector: string, tuple: TuplaDay) {
